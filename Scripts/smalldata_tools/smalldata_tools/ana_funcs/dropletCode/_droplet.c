...
extern void raw_dropletanal(long int *img, long int *bimg, long int *npix,
        double *xcen, double *ycen, double *xsig,double  *ysig,double  *xysig, long int *adus, long int *idlist, int npeak,int
        ncol, int nrow);

//Should adus vector be float or double?
void raw_dropletanal(long int *img, long int *bimg, long int *npix, double 
        *xcen, double *ycen, double *xsig,double *ysig, double *xysig, long int *adus,long int *idlist, int npeak,int
        ncol, int nrow) {
	int i,id;
	...
	for (i=0;i<nrow*ncol;i++) {
		if (bimg[i]) { 
			...
			xsig[pos-1]+=x*x*(double)(img[i]);
			ysig[pos-1]+=y*y*(double)(img[i]);
			xysig[pos-1]+=x*y*(double)(img[i]);
			adus[pos-1]+=img[i];
			}
		}
	for (i=0;i<npeak;i++) {
		if (adus[i]) {
			...
			xsig[i] =1/12.0+xsig[i]/(double)(adus[i])-xcen[i]*xcen[i];
			ysig[i] =1/12.0+ysig[i]/(double)(adus[i])-ycen[i]*ycen[i];
			xysig[i] =xysig[i]/(double)(adus[i])-xcen[i]*ycen[i];
			}
		else {
